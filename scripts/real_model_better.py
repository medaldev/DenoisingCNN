...
class FilterBlockResidual(nn.Module):
    def __init__(self, in_channels, out_channels, use_fft: bool = False, residual: bool = True):
        ...
        self.bn2 = nn.LayerNorm([32, 10, 10, 10])
        self.bn3 = nn.LayerNorm([64, 10, 10, 10])
        self.bn4 = nn.LayerNorm([64, 10, 10, 10])
        self.bn5 = nn.LayerNorm([64, 10, 10, 10])
        self.bn6 = nn.LayerNorm([32, 10, 10, 10])
        self.bn7 = nn.LayerNorm([16, 10, 10, 10])
...
class DenoiserBlock(nn.Module):
    def __init__(self, in_channels, num_par_filters, out_channels):
        ...
        self.par_fil = nn.ModuleList([
            FilterBlockResidual(in_channels=in_channels, out_channels=3)
            for i in range(num_par_filters)
        ])
        self.reducer = FilterBlockResidual(in_channels=3 * num_par_filters, out_channels=out_channels)
...
if __name__ == '__main__':
    ...
    model = FilterBlockResidual(in_channels=90, out_channels=3).to(device, dtype=dtype)
    # model.eval()
    #
