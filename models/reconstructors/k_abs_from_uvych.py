...
class K_abs_Uvych_k0_v1(nn.Module):
    def __init__(self, size_uvych_parts: int, size_k_res: int):
        super(K_abs_Uvych_k0_v1, self).__init__()
...
def test():
    ...
    model = K_abs_Uvych_k0_v1(3200, 800).to(device).eval()
    x1 = torch.randn(1, 1, 3200, device=device)
    x2 = torch.randn(1, 1, 3200, device=device)
